...
def get_or_create_spot(supplier_id, spot_number):
    """Находит существующее место по номеру у поставщика или создаёт новое."""
    with get_connection() as conn:
        c = conn.cursor()
        r = c.execute('SELECT id FROM parking_spots WHERE supplier_id=? AND spot_number=? AND is_available=1',
                      (supplier_id, spot_number)).fetchone()
        if r: return r[0]
        c.execute('INSERT INTO parking_spots (supplier_id,spot_number,price_per_hour) VALUES (?,?,0)',
                  (supplier_id, spot_number))
        return c.lastrowid
...
def get_spot_availabilities(sid):
    with get_connection() as conn:
        return [dict(r) for r in conn.cursor().execute(
            "SELECT * FROM spot_availability WHERE spot_id=? AND end_time>datetime('now','localtime') ORDER BY start_time ASC",(sid,)).fetchall()]
...
def admin_edit_booking_hours(bid, paid_hours):
    """Устанавливает оплаченные часы. Остаток — свободный слот."""
    with get_connection() as conn:
        c = conn.cursor()
        booking = c.execute('SELECT * FROM bookings WHERE id=?',(bid,)).fetchone()
        if not booking: return False
        book_start = datetime.fromisoformat(booking['start_time'])
        book_end = datetime.fromisoformat(booking['end_time'])
        new_end = book_start + timedelta(hours=paid_hours)
        if new_end >= book_end: return True
        spot_id = booking['spot_id']
        aid = booking['availability_id']
        from utils import calculate_price
        tp = calculate_price(book_start, new_end)
        c.execute('UPDATE bookings SET end_time=?, total_price=? WHERE id=?',
                  (new_end.strftime("%Y-%m-%d %H:%M:%S"), tp, bid))
        c.execute('INSERT INTO spot_availability (spot_id,start_time,end_time,is_booked) VALUES (?,?,?,0)',
                  (spot_id, new_end.strftime("%Y-%m-%d %H:%M:%S"), book_end.strftime("%Y-%m-%d %H:%M:%S")))
        c.execute('UPDATE spot_availability SET end_time=? WHERE id=?',
                  (new_end.strftime("%Y-%m-%d %H:%M:%S"), aid))
        _log(c, 'booking_edited', booking_id=bid, details=f"paid_hours={paid_hours}")
        return True
