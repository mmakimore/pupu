...
from config import BANKS, MAX_ACTIVE_BOOKINGS, MAX_SPOTS_PER_USER, ABOUT_TEXT, RULES_TEXT
...
async def confirm_booking(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await state.clear(); await callback.message.edit_text("❌ Слот уже занят."); return
    user = db.get_user_by_telegram_id(callback.from_user.id)
    await state.clear()
    ...
    await callback.message.edit_text(
        ...
        f"⏱ {h:.1f}ч | 💰 {rate}₽/ч = <b>{data['total_price']}₽</b>\n\n"
...
async def add_spot_start(message: Message, state: FSMContext):
    ...
    if existing:
        ...
        await message.answer("🏠 <b>Номер места</b>:", reply_markup=get_cancel_menu_keyboard(), parse_mode="HTML")
...
async def new_spot(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await callback.message.edit_text("🏠 <b>Номер нового места</b>:", parse_mode="HTML")
    await state.set_state(AddSpotStates.waiting_spot_number)
...
async def sp_num(message: Message, state: FSMContext):
    ...
    await message.answer("📅 <b>Дата начала</b>:", reply_markup=get_dates_keyboard("start_date"), parse_mode="HTML")
...
async def sp_sd(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("⏰ Время начала:", reply_markup=get_time_slots_keyboard("start_time"))
...
async def sp_sd_m(message: Message, state: FSMContext):
    ...
    await message.answer("⏰ Время начала:", reply_markup=get_time_slots_keyboard("start_time"))
    await state.set_state(AddSpotStates.waiting_start_time)
...
async def sp_ed(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("⏰ Время окончания:", reply_markup=get_time_slots_keyboard("end_time"))
...
async def sp_ed_m(message: Message, state: FSMContext):
    ...
    await message.answer("⏰ Время окончания:", reply_markup=get_time_slots_keyboard("end_time"))
    await state.set_state(AddSpotStates.waiting_end_time)
...
async def spot_detail(callback: CallbackQuery, state: FSMContext):
    ...
    for a in avails:
        ...
        status = "🔴 занят" if a['is_booked'] else "🟢 свободен"
        at += f"\n📅 {format_datetime(s)} — {format_datetime(e)} [{status}]"
    if not at: at = "\nНет активных слотов"
...
async def aslot_sd(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("⏰ Время начала:", reply_markup=get_time_slots_keyboard("aslot_st"))
...
async def aslot_sd_m(message: Message, state: FSMContext):
    ...
    await message.answer("⏰ Время начала:", reply_markup=get_time_slots_keyboard("aslot_st"))
    await state.set_state(AddSlotStates.waiting_start_time)
...
async def aslot_ed(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("⏰ Время окончания:", reply_markup=get_time_slots_keyboard("aslot_et"))
...
async def aslot_ed_m(message: Message, state: FSMContext):
    ...
    await message.answer("⏰ Время окончания:", reply_markup=get_time_slots_keyboard("aslot_et"))
    await state.set_state(AddSlotStates.waiting_end_time)
